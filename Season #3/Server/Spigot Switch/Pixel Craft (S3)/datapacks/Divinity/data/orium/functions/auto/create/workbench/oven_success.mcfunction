...
execute positioned ~ ~1 ~ as @e[type=item,distance=..0.1,nbt={Item:{id:"minecraft:furnace",tag:{display:{Name:'{"italic":false,"text":"Thermic Oven"}'}}}}] run tag @s add craftedItem
...
execute unless entity @s[tag=itemExists] run summon minecraft:item ~ ~1 ~ {Item:{id:"minecraft:furnace",Count:1b,tag:{display:{Name:'{"italic":false,"text":"Thermic Oven"}'}}}}
