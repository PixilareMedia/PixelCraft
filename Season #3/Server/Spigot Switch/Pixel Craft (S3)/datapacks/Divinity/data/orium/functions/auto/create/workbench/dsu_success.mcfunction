...
execute positioned ~ ~1 ~ as @e[type=item,distance=..0.1,nbt={Item:{id:"minecraft:hopper",tag:{CustomModelData:2,display:{Name:'{"italic":false,"text":"Deep Storage Unit"}'}}}}] run tag @s add craftedItem
...
execute unless entity @s[tag=itemExists] run summon minecraft:item ~ ~1 ~ {Item:{id:"minecraft:hopper",Count:1b,tag:{CustomModelData:2,display:{Name:'{"italic":false,"text":"Deep Storage Unit"}'}}}}
