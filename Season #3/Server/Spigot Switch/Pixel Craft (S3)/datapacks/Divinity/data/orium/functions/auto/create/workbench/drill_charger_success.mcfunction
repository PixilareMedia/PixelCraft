...
execute positioned ~ ~1 ~ as @e[type=item,distance=..0.1,nbt={Item:{id:"minecraft:dropper",tag:{CustomModelData:2,display:{Name:'{"italic":false,"color":"dark_red","text":"Laser Drill Arc Generator"}'}}}}] run tag @s add craftedItem
...
execute unless entity @s[tag=itemExists] run summon minecraft:item ~ ~1 ~ {Item:{id:"minecraft:dropper",Count:1b,tag:{CustomModelData:2,display:{Name:'{"italic":false,"color":"dark_red","text":"Laser Drill Arc Generator"}'}}}}
